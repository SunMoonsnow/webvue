...
export default {
  ...
  created(){
  	...
  	this.smid = this.$route.params.sid;
  	this.mid = this.$route.params.mid;

  	//1、获取vKey
  	let url = api.vKeyApi + `&songmid=${this.smid}&filename=C400${this.sid}.m4a`;
    console.log(url);
    console.log(this.smid);
  	jsonp(url,{param:'callback'},(err,data)=>{
  		let vkey = data.data.items[0].vkey;
  		//2、使用 smid和vKey取得歌曲播放地址
  		this.src = `http://dl.stream.qqmusic.qq.com/C400${this.sid}.m4a?vkey=${vkey}&guid=7120953682&uin=0&fromtag=66`;
      let pic = "<img src='" + this.src + "'></img>"
  	});
